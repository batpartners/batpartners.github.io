...
ABB 워크오브젝트(워크피스 좌표계)를 지정할 수 있다. 월드 좌표계에서 작업대의 로컬 좌표계로 사용자가 정의할 수 있다. 사용자 프레임(uframe) 및 오브젝트 프레임(oframe)은 워크오브젝트 기준으로 타겟평면 좌표를 재정의할 수 있다. 또한 컴포넌트 아이콘을 우클릭하여 Movable(외부축에 의해 움직이는 워크오브젝트), Fixed(고정된 일반 워크오브젝트) 두 모드를 취사 선택할 수 있다.
