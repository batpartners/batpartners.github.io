...
두 커브로부터 열선 절단용 툴패스를 생성하는 컴포넌트이다. 사용자는 타겟 평면을 생성할 포인트 개수와, 두 커브 사이의 파라미터 값(기본 : 0.5), 선택 옵션으로 열선 툴을 부드럽게 조정할 수 있는 어트랙터 값을 설정한다. 이외의 열선 경로 방향, 툴 방향, 진입 간격의 세부 설정이 가능하다.
...
| **Spans** | Curve | 두 레일 커브를 연결하여 ruled surface를 구성하는 직선 세그먼트. |